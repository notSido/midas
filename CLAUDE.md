...
- **One slice per PR onto `main`.** Branch, implement, verify locally, open a PR.
  ...
  green before merge. **Doc-only** PRs skip the CI wait — dual-review + a local
  no-hype check (`grep -rnE '🎉|✅|SUCCESS|BREAKTHROUGH|MAJOR|FINALLY' <file> |
  grep -vF '🎉|✅|SUCCESS|BREAKTHROUGH|MAJOR|FINALLY'` returns nothing), then merge.
- **Every PR gets two independent reviews before merge:** an **Opus** review
  ...
  run concurrently on the checked-out branch. Relay a side-by-side comparison,
  address real findings (fix + re-verify), then merge. The human merges PRs; the
  assistant does not self-merge.
