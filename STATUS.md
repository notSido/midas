...
for kernel32 (starting at the `e_lfanew` read), so the synthetic-module machinery
must be generalised to any loaded DLL (mapped image + export directory), seeded
with each DLL's export names; then
